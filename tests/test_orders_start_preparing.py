"""Integration tests for POST /api/orders/{id}/start-preparing (ticket S3-04)."""
import threading
...
def test_start_preparing_confirmed_order_returns_200(seeded_client: TestClient) -> None:
...
def test_start_preparing_event_recorded(seeded_client: TestClient, db_engine: Engine) -> None:
    diner_token = _get_diner_token(seeded_client)
    staff_token = _get_staff_token(seeded_client)
    order_id = _create_order(seeded_client, diner_token)
    _confirm_order(seeded_client, order_id, staff_token)
    ...
    with SASession(db_engine) as s:
        events = s.scalars(
            select(OrderEvent).where(
                ...
                OrderEvent.to_status == OrderStatus.PREPARING,
            )
        ).all()
...
def test_start_preparing_already_preparing_returns_409(seeded_client: TestClient) -> None:
    diner_token = _get_diner_token(seeded_client)
    staff_token = _get_staff_token(seeded_client)
    order_id = _create_order(seeded_client, diner_token)
    _confirm_order(seeded_client, order_id, staff_token)
...
def test_start_preparing_order_not_confirmed_returns_409(
...
def test_start_preparing_order_not_found_returns_404(seeded_client: TestClient) -> None:
    staff_token = _get_staff_token(seeded_client)

    r = seeded_client.post(
        "/api/orders/nonexistentid00000000000000000/start-preparing",
        headers={"Authorization": f"Bearer {staff_token}"},
    )
    assert r.status_code == 404


def test_start_preparing_cross_restaurant_returns_403(
    seeded_client: TestClient, db_engine: Engine
) -> None:
    diner_token = _get_diner_token(seeded_client)
    staff_token = _get_staff_token(seeded_client)
...
def test_concurrent_start_preparing_one_200_one_409(seeded_client: TestClient) -> None:
    """Two start-preparing requests on the same CONFIRMED order: exactly one 200, one 409."""
    diner_token = _get_diner_token(seeded_client)
    staff_token = _get_staff_token(seeded_client)
    order_id = _create_order(seeded_client, diner_token)
    _confirm_order(seeded_client, order_id, staff_token)
    ...
    t1 = threading.Thread(target=prepare_first)
    t2 = threading.Thread(target=prepare_second)
    t1.start()
    t2.start()
    t1.join()
    t2.join()
