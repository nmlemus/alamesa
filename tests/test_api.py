...
def test_register_diner(client: TestClient) -> None:
    r = client.post(
        "/api/diners/register",
        json={"email": "ana@example.com", "name": "Ana García", "password": "s3cr3t"},
    )
...
def test_register_diner_duplicate_email(client: TestClient) -> None:
    payload = {"email": "dup@example.com", "name": "User", "password": "pass"}
    client.post("/api/diners/register", json=payload)
    r = client.post("/api/diners/register", json=payload)
...
def test_order_full_lifecycle(seeded_client: TestClient) -> None:
    ...
    for status in ("confirmed", "ready", "closed"):
...
def test_invalid_status_transition(seeded_client: TestClient) -> None:
    ...
    # Cannot jump pending → ready (skipping confirmed)
    r = seeded_client.patch(
...
def test_invalid_status_value(seeded_client: TestClient) -> None:
    r = seeded_client.patch("/api/orders/1/status", json={"status": "cooking"})
