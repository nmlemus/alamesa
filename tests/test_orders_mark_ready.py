"""Integration tests for POST /api/orders/{id}/mark-ready (ticket S3-04)."""
import threading
...
def _start_preparing(client: TestClient, order_id: str, staff_token: str) -> None:
    r = client.post(
...
def test_mark_ready_preparing_order_returns_200(seeded_client: TestClient) -> None:
    ...
    _start_preparing(seeded_client, order_id, staff_token)
...
def test_mark_ready_event_recorded(seeded_client: TestClient, db_engine: Engine) -> None:
    diner_token = _get_diner_token(seeded_client)
    staff_token = _get_staff_token(seeded_client)
    order_id = _create_order(seeded_client, diner_token)
    _confirm_order(seeded_client, order_id, staff_token)
    ...
    with SASession(db_engine) as s:
        events = s.scalars(
            select(OrderEvent).where(
                ...
                OrderEvent.from_status == OrderStatus.PREPARING,
                OrderEvent.to_status == OrderStatus.READY,
            )
        ).all()
...
def test_mark_ready_already_ready_returns_409(seeded_client: TestClient) -> None:
    diner_token = _get_diner_token(seeded_client)
    staff_token = _get_staff_token(seeded_client)
    order_id = _create_order(seeded_client, diner_token)
    _confirm_order(seeded_client, order_id, staff_token)
...
def test_mark_ready_order_not_preparing_returns_409(
...
def test_mark_ready_order_not_found_returns_404(seeded_client: TestClient) -> None:
    staff_token = _get_staff_token(seeded_client)

    r = seeded_client.post(
        "/api/orders/nonexistentid00000000000000000/mark-ready",
        headers={"Authorization": f"Bearer {staff_token}"},
    )
    assert r.status_code == 404


def test_mark_ready_cross_restaurant_returns_403(
    seeded_client: TestClient, db_engine: Engine
) -> None:
    diner_token = _get_diner_token(seeded_client)
    staff_token = _get_staff_token(seeded_client)
...
def test_mark_ready_no_auth_returns_401(seeded_client: TestClient) -> None:
    diner_token = _get_diner_token(seeded_client)
    order_id = _create_order(seeded_client, diner_token)

    r = seeded_client.post(f"/api/orders/{order_id}/mark-ready")
    assert r.status_code == 401


def test_mark_ready_diner_token_returns_401(seeded_client: TestClient) -> None:
    diner_token = _get_diner_token(seeded_client)
    order_id = _create_order(seeded_client, diner_token)
...
def test_concurrent_mark_ready_one_200_one_409(seeded_client: TestClient) -> None:
    """Two mark-ready requests on the same PREPARING order: exactly one 200, one 409."""
    diner_token = _get_diner_token(seeded_client)
    staff_token = _get_staff_token(seeded_client)
    order_id = _create_order(seeded_client, diner_token)
    _confirm_order(seeded_client, order_id, staff_token)
    ...
    t1 = threading.Thread(target=ready_first)
    t2 = threading.Thread(target=ready_second)
    t1.start()
    t2.start()
    t1.join()
    t2.join()
