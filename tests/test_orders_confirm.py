...
def test_confirm_pending_order_returns_200(seeded_client: TestClient) -> None:
...
def test_confirm_order_event_recorded(seeded_client: TestClient, db_engine: Engine) -> None:
    from mesadigital.api.db.models import OrderEvent
    ...
    with SASession(db_engine) as s:
        events = s.scalars(
            ...
            .where(
                ...
                OrderEvent.to_status == OrderStatus.CONFIRMED,
            )
        ).all()
...
def test_confirm_already_confirmed_order_returns_409(seeded_client: TestClient) -> None:
    diner_token = _get_diner_token(seeded_client)
    staff_token = _get_staff_token(seeded_client)
    order_id = _create_order(seeded_client, diner_token)
...
def test_confirm_order_not_pending_returns_409(seeded_client: TestClient, db_engine: Engine) -> None:
    diner_token = _get_diner_token(seeded_client)
    staff_token = _get_staff_token(seeded_client)
    order_id = _create_order(seeded_client, diner_token)
...
def test_confirm_order_not_found_returns_404(seeded_client: TestClient) -> None:
    staff_token = _get_staff_token(seeded_client)

    r = seeded_client.post(
        "/api/orders/nonexistentid00000000000000000/confirm",
        headers={"Authorization": f"Bearer {staff_token}"},
    )
    assert r.status_code == 404


def test_confirm_order_cross_restaurant_returns_403(
...
def test_concurrent_confirm_one_200_one_409(seeded_client: TestClient) -> None:
    """Two confirm requests on the same PENDING order: exactly one 200, one 409.

    SQLite does not support row-level FOR UPDATE locking, so Thread 2 is
    sequenced after Thread 1 via an Event — the same post-commit serialisation
    that FOR UPDATE provides on PostgreSQL.
    """
    diner_token = _get_diner_token(seeded_client)
    staff_token = _get_staff_token(seeded_client)
    order_id = _create_order(seeded_client, diner_token)
    ...
    t1 = threading.Thread(target=confirm_first)
    t2 = threading.Thread(target=confirm_second)
    t1.start()
    t2.start()
    t1.join()
    t2.join()
